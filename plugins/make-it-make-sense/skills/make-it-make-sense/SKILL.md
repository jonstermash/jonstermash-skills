...
- **Name the source or drop the appeal.** Between a real anchor and none sits the tell that survives every other check: the gesture at authority that names nothing — "studies show," "industry best practice," "as many have noted," "research suggests." It reads as sourced, so it passes a fabrication check that only looks for invented specifics, and it can't be verified because there's nothing to verify. Name the source or cut the appeal and make the claim on its own. (`references/framing-tells.md`)
...
- **Headings name their subject.** Default to a **categorical noun phrase** — what the section is *about*, not a sentence restating what it *says*. "Method," not "What we measured." "Rollback plan," not "How we'd back this out if it fails." A heading is an address the reader navigates by; a heading that delivers the content makes them read it twice and stops the outline working as an index. A claim heading ("Churn is a pricing problem, not a product one") is the deliberate exception — earned where the heading may be the only thing read, like a slide or an email subject, and paid off by the body. Never spin forced onto reference content, and never an editorialized read of a result. (`references/titles.md`)
...
- **Kill redundancy and contradiction.** Sweep every section against every other, both questions: did the reader already get this (merge/cut — emphasis comes from placement, not repetition), and do these two claims disagree (a contradiction reads clean locally and only surfaces when the pair is held together)? An executive summary earns its place only at a **different altitude** than the detail it precedes — a one-line verdict plus what it sets up, not a re-telling. Usual offenders: a chart and a table showing the same numbers, an intro restating the executive summary, a "next steps" repeating the recommendation, two diagrams making the same point, and a closing passage that explains what the piece *meant* — the significance coda, which restates the point the piece already made and adds a moral to it. Leading with the answer is the rule; stating it a second time as a lesson is the failure.

**Gate:** read the headings alone, in order. The outline reads as an **index, not a summary** — each heading names its own subject, none restates its section's opening line, and any that makes a claim instead is one you chose deliberately and the body pays off. Each must also be **specific to this piece**: a heading that could sit unchanged over someone else's document on another topic ("Overview," "Details," "Next Steps") is too generic, and no two may be interchangeable with each other. Zero colons-as-crutch, zero filler, no pronoun whose antecedent lives outside the outline.
